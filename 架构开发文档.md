# AI Agent Framework — 开发实现文档 v2.3
...
* `_build_effective_config(agent, active_skill) -> AgentConfig`
...
3. 记忆层默认只做 Saved Memories，不做全文检索。
4. 默认记忆实现只依赖数据库，不引入 `rank_bm25`。
5. `BaseMemoryManager` 与 `MemoryStoreProtocol` 是记忆层主干。
6. 去重与合并逻辑只在 `MemoryManager` 中实现，不进入 store。
7. 记忆层不做 prompt 格式化，格式化由上下文层完成。
8. Session History 必须由 `SessionState` 唯一持有。
9. 工具权限优先级固定为：`CapabilityPolicy` → `ScopedToolRegistry` → `on_tool_call_requested()`。
10. Skill override 只作用于当前 run 的 `effective config`。
11. 子 Agent 默认不继承父 Skill override。
12. `spawn_agent` 返回给 LLM 的必须是摘要结果。
13. `SHARED_WRITE` 只能通过父 `MemoryManager.remember()` 写回。
14. 默认记忆风格要接近 GPT Saved Memories，而不是知识库检索系统。
15. 用户必须能够列出、删除、启停和钉住自己的 Saved Memories。
