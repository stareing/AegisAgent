...
class LiteLLMAdapter(BaseModelAdapter):
    """Default model adapter backed by litellm."""
    ...
    async def complete(
        ...
        """Send a non-streaming completion request with retry."""
    ...
    def count_tokens(self, messages: list[Message]) -> int:
        """Count tokens using tiktoken when available, otherwise rough estimate."""
        try:
            import tiktoken
            ...
            text = "".join(
                (m.content or "") + json.dumps([tc.model_dump() for tc in (m.tool_calls or [])], default=str)
                for m in messages
            )
            return len(text) // 4
    ...
    def _parse_response(self, raw: Any) -> ModelResponse:
        """Convert a litellm response into our ModelResponse."""
        ...
        if hasattr(raw, "system_fingerprint"):
            raw_meta["system_fingerprint"] = raw.system_fingerprint
    ...
    def _parse_tool_calls(
        self, raw_tool_calls: list[Any] | None
    ) -> list[ToolCallRequest]:
        """Parse tool calls from the raw response, handling malformed JSON gracefully."""
        ...
        for tc in raw_tool_calls:
            ...
            try:
                ...
                logger.warning(
                    "Failed to parse tool call arguments for '%s': %s — defaulting to empty dict",
                    name,
                    args_str,
                )
                arguments = {}
    ...
    def _parse_chunk(chunk: Any) -> ModelChunk:
        """Convert a single streaming chunk into a ModelChunk."""
